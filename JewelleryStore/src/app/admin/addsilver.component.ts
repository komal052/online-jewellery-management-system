...
@Component({
  selector: 'app-addsilver',
  templateUrl: './addsilver.component.html',
  styles: [],
  encapsulation : ViewEncapsulation.None,
})
export class AddsilverComponent implements OnInit {
  ...
  insertDetails(form:NgForm){
    this.service.InsertSilver(this.silverData).subscribe(res=>{
      ...
      if(this.insertResponce.result === "success"){
        this.resetForm(form);
        this.router.navigate(['/admin/showsilver'],{relativeTo:this.route})
      }else{
        console.log(res);
      }
    },err=>{
   ...
   updateDetails(form:NgForm){
    this.service.UpdateSilver(this.silverData).subscribe(res=>{
      ...
      if(this.insertResponce.result === "success"){
        this.resetForm(form);
        this.router.navigate(['/admin/showsilver']).then(() => {
