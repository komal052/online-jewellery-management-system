...
@Component({
  selector: 'app-m-add-user',
  templateUrl: './m-add-user.component.html',
  styles: [
  ]
})
export class MAddUserComponent implements OnInit {
  ...
  insertDetails(form:NgForm){
    ...
    this.service.InsertUser(formData).subscribe(res=>{
      ...
      if(this.insertResponce.result === "success"){
        this.resetForm(form);
        this.router.navigate(['/manager/m_show_registration']).then(() => {
