...
public class city_tableDB : clsDB_Operation
{
    ...
    public city_tableEntities OnGetData(int ID)
    {
        ...
        try
        {
            strQ = @"SELECT c.*,s.state_name FROM [city_master] c  
            JOIN [state_master] s  ON c.[state_id_fk]=s.[state_id_pk] 
             WHERE [city_id_pk]= @city_id_pk";
    ...
    public List<city_tableEntities> OnGetListState(int ID)
    {
        Exception exForce;
        //IDataReader oReader;
        DataTable dtTable;
        ...
        try
        {
            ...
            strQ = @"SELECT c.*,s.state_name FROM [city_master] c  
             JOIN [state_master] s  ON c.[state_id_fk]=s.[state_id_pk] where 
                [state_id_fk]=@state_id_fk";

            OnClearParameter();
            AddParameter("state_id_fk", SqlDbType.Int, 2, ID, ParameterDirection.Input);
            dtTable = OnExecQuery(strQ, "list").Tables[0];
